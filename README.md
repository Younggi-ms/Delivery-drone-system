...
### 디지털 포렌식과의 연결
**자율 비행 시스템의 멀티센서 융합 및 GPS/IMU 신뢰성 문제를 직접 경험**한 것은 자율 이동체 포렌식 연구의 직접적 출발점입니다.

- **자율 드론 사고 재현**: PID 제어 루프 + FSM 상태 전환 로그 구조를 직접 설계한 경험 → 드론 사고 발생 시 비행 데이터 재구성 및 원인 분석에 동일한 구조 적용
- **GPS 스푸핑 탐지**: IMU 진동 결합(vibration coupling)으로 인한 센서 오류를 실제 경험 → GPS 신호 위변조 vs 기계적 노이즈 구별 분석의 기초
- **비행 로그 무결성 분석**: Serial2 기반 GPS 데이터 스트림의 무결성 검증 구조 설계 경험 → 드론 블랙박스 데이터 포렌식 분석 역량
- **RC 프로토콜 역분석**: 커스텀 인코딩 프로토콜(`(char)(val/5+52)`) 직접 설계 경험 → 미지의 RC 통신 프로토콜 역분석 능력

### IoT/드론 보안 취약점 관점
- RC 수신기-메인 보드 간 통신이 평문 Serial 전송 → **중간자 공격(MitM)** 및 **명령 위변조** 취약점 직접 인지
- GPS 수신 데이터 검증 로직 부재 → **GPS 스푸핑 공격**에 무방비한 구조를 구현자 관점에서 파악

### AI 확장 가능성
- IMU + GPS 시계열 데이터에 **딥러닝 이상 탐지 모델 적용** → 정상 비행 vs 사이버 침해(GPS 스푸핑, RC 하이재킹) 패턴 분류 연구
- 착륙장 인식 컴퓨터 비전 알고리즘 → **포렌식 증거 영상 분석 AI** (드론 촬영 영상 기반 사건 재현)로 확장 가능
- 비행 로그 데이터 기반 **자율 이동체 행동 분석 AI** → 무인기 테러·밀수 등 보안 사건 수사 지원 연구 주제
